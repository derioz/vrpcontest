...
const INITIAL_CHANGELOGS: ChangelogEntry[] = [
  {
    ...
    version: 'v1.9.7',
    title: 'SeraUI Official User Profile Dropdown Suite & Multi-Contest Champion Badge System',
    category: 'UI/UX',
    description: `• Multi-Contest Champion Badge Suite Overhaul: Re-engineered the victory badge system into a clean, component-based multi-tier progression (1x Gold Winner, 2x Platinum Champion, 3-4x Royal Amethyst Grand Champion, and 5+ Mythic Legend) built on top of a reusable Badge primitive with subtle glassmorphic glow, metallic borders, and crisp typography.
• High-Density Compact Badge Telemetry: Upgraded compact badge rendering across photo feeds, profile cards, and winner directories with micro-pill numeric counters (e.g. [🏆 1], [👑 2x], [👑 3x], [🔥 5x]) preventing line breaks while clearly communicating historical contest accomplishments.
• Reusable Badge Primitive Component: Introduced Badge in components/ui/badge.tsx with multi-variant styling (gold, platinum, amethyst, mythic, success, warning, destructive, outline), multiple sizes (xs to lg), and interactive hover micro-animations.
• Official SeraUI Profile Dropdown Architecture: Re-engineered the platform profile dropdown to match the official Sera UI Dropdown specification (https://seraui.com/docs/dropdown), featuring a dedicated modular UserProfileDropdown component with deep frosted glassmorphism, responsive spring animations, and refined micro-interactions.
